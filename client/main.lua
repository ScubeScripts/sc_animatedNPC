			...
			for _, location in ipairs(pedGroup.locations) do
				...
				if nearby and not spawned then
					...
					if IsModelInCdimage(model) then
						...
						if pedGroup.props then
							for _, prop in ipairs(pedGroup.props) do
								local propHash = GetHashKey(prop.name)
								...
								local propEntity = CreateObject(propHash, 0.0, 0.0, 0.0, true, true, false)
								AttachEntityToEntity(propEntity, npc, GetPedBoneIndex(npc, prop.bone), prop.offset.x, prop.offset.y, prop.offset.z, prop.rotation.x, prop.rotation.y, prop.rotation.z, true, true, false, true, 1, true)
